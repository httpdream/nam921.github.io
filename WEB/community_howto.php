...
<html>
    ...
    <body>
        <span id="title">공략게시판</span>
        <span ><a href="write_howto.html" id="btn">글쓰기</a></span>
        <br/>
...
$result='addr';
             if(isset($_GET['page'])){
                 ...
                 $prev = ($f-1)*10;
                 $next = $f*10;
                $result = mysql_query("select* from board_howto order by idx desc limit $prev,10");
             }

            else $result = mysql_query('select* from board_howto order by idx desc');
             while($row = mysql_fetch_row($result)){
                 echo "<tr class='body'>";
                 for($i = 0; $i<3; $i++)
                    echo "<td class='section1'><a href='view_howto.php?id=$row[0]'>$row[$i]</a><br></td>";
                 echo "<td class='section1'><a href='view_howto.php?id=$row[0]'>$row[4]</a><br></td>";
                 echo "</tr>";
             }
        ...
        <div class="center">
            <a href='community_howto.php?page=<?php
...
?>
        ...
        <span>
            <?php
$result = mysql_query("select* from board_howto");
$row = ceil(mysql_num_rows($result)/10);

for($i = 1; $i<=$row; $i++)
    echo "<a href='community_howto.php?page=$i'> $i </a>";
            ?>
